...
class GeoSIMConformalSpatialRegression:
    def __init__(self, predict_f: Callable, nonconformity_score_f: Callable = None,
        ...
        if standardize_weights:
            stds = np.std(self.X_calib_weight, axis=0)
            valid_cols = stds > 1e-8
            self.X_calib_weight = self.X_calib_weight[:, valid_cols]
            self.X_test_weight = self.X_test_weight[:, valid_cols]
    ...
    def _weighted_quantile(self, scores: np.ndarray, alpha: float = 0.1, weights: np.ndarray = None):
        ...
        normalized_cumsum_weights = cumsum_weights / (cumsum_weights[-1] + 1e-8)

...
class GeoSIMCPzhao:
    def __init__(self, predict_f: Callable, nonconformity_score_f: Callable = None,
        ...
        if standardize_weights:
            self.ranges = np.ptp(self.X_calib_weight, axis=0)  # feature-wise range
            self.ranges[self.ranges == 0] = 1e-8  # avoid zero range
        else:
            self.ranges = np.ones(self.X_calib_weight.shape[1])
    ...
    def _custom_feat_distance(self, x_test, x_calib):
        diffs = np.abs(x_calib - x_test)
        scaled = 1 - (diffs / self.ranges)
        similarity = np.min(scaled, axis=1)
        return 1 - similarity

    def _kernel_smoothing_joint_zhao2024(self, z_test, z_calib, x_test, x_calib, bandwidth, lam=1.0):
        d_geo = np.linalg.norm(z_calib - z_test, axis=1)
        d_feat = self._custom_feat_distance(x_test, x_calib)
    ...
    def predict_geoconformal_uncertainty(self):
        ...
        for i in range(len(self.coord_test)):
            ...
            weights = self._kernel_smoothing_joint_zhao2024(
                z_test=p,
        ...
        self.uncertainty = np.mean(self.geo_uncertainty)
